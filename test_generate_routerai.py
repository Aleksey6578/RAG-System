...
# [FIX-§6.2] ИСПРАВЛЕНО (отчёт §6.2): поднят с 15 до 20.
# ОПК-1 (86) и УК-1 (87) не достигали порога 100 вопросов.
# 20 × 3 ранга × 3 раздела = 180 вопросов/компетенцию при полном покрытии.
MIN_PER_RANK    = 20
MIN_PER_SECTION = 30
MIN_PER_COMP    = 100
...
def parse_rpd_sections(rpd_path: Path) -> list[dict]:
    ...
    # обеспечение» и «материально-техническое обеспечение» ��опадали в темы,
    ...
    _FOS_STOPWORDS = (
        ...
        # [FIX-§BL1] Заголовки раздела «Учебно-методическое обеспечение»:
        # «об обеспеченности» и «основной и дополнительной» — это подзаголовки
        # списков литературы, не темы дисциплины.
    ...
    # [FIX-§BL1] Строки вида «(38050) Название темы» — темы с кодом дисциплины
    # в скобках. Попадают в топики из таблиц тематического плана, но содержат
    # только технический идентификатор и не несут смысловой нагрузки для LLM.
    ...
    for text in paragraphs:
        ...
        if current_section:
            ...
            if _NUMERIC_HEADER_RE.match(text):
                continue
            # [FIX-§BL1] Пропускаем темы-заголовки с кодом дисциплины: «(38050) ...»
