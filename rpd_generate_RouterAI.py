...
def fill_annotation_table(
    ...
    comp_lines = []
    # [FIX-ANN] Ранее индикатор был хардкодом «Применяет методы и инструменты
    # дисциплины» для всех компетенций. Теперь берётся первый З-outcome с ротацией
    # по индексу, чтобы каждая компетенция получила уникальный индикатор.
    z_outcomes_list = [t for ot, t in outcomes if ot == "З"] if outcomes else []
    for i, (code, desc) in enumerate(competencies):
        comp_lines.append(f" {code} {desc}:")
        ind_text = (
            z_outcomes_list[i % len(z_outcomes_list)][:80]
            if z_outcomes_list else "основные методы и инструменты дисциплины"
        )
        comp_lines.append(f"-{code}.1 Знает {ind_text}")
    comp_text = "\n".join(comp_lines)
...
def fill_doc_header(doc: Document, discipline: str, code: str,
    ...
    # Плейсхолдер [] после такого заголовка — это перечень вопросов/заданий,
    # а не название дисциплины. Пропускаем здесь, заполняет fill_appendix_v().
    _ASSESSMENT_HEADERS = {
...
def fill_outcomes_table(doc: Document, competencies: list, outcomes: list):
    ...
    [Фикс T5-vMerge] Шаблон использует vMerge: col0 (Шифр компетенции) и col1
    (Индикаторы) объединены по вертикали на 3 строки (З/У/В) для каждой компетенции.
    Структура: row_Z (vMerge=restart), row_У (vMerge=cont), row_В (vMerge=cont).
    Прежний код писал code/indicator в каждую из 3 строк — это рвало объединение.
    Теперь code+indicator_block пишутся только в строку restart, cont-строки
    получают пустые col0/col1 (Word визуально показывает содержимое restart-ячейки).

    [Фикс №7] indicator_block — три подиндикатора через \\n в одной ячейке col1.
    _set_cell_xml создаёт отдельные <w:p> для каждой строки.

    [Фикс №2] Если LLM вернул меньше items чем компетенций — дополняем fallback-
    формулировками, устраняя wrap-around idx % len(items).
    """
    ...
    # [Фикс №2] Дополняем items до len(competencies) производными формулировками
    for _i in range(len(z_items), len(competencies)):
        _, _d = competencies[_i]
        z_items.append(f"теоретические основы и методы: {re.sub(r'^Способен ', '', _d, flags=re.IGNORECASE)[:60]}")
    for _i in range(len(u_items), len(competencies)):
        _, _d = competencies[_i]
        u_items.append(f"применять методы в области: {re.sub(r'^Способен ', '', _d, flags=re.IGNORECASE)[:60]}")
    for _i in range(len(v_items), len(competencies)):
        _, _d = competencies[_i]
        v_items.append(f"навыками работы с инструментами: {re.sub(r'^Способен ', '', _d, flags=re.IGNORECASE)[:60]}")
    ...
    # ── Находим строки-placeholder и шаблонную группу (restart+cont+cont) ───
    tbl_xml     = table._tbl
    ph_trs      = [tr for tr in list(tbl_xml) if _is_placeholder_row(tr)]
    if not ph_trs:
        print("  ⚠️  [fill_outcomes] Строк с [] не найдено")
        return

    # Первые 3 placeholder-строки → шаблонная группа (restart/cont/cont).
    # Если шаблон изменился и строк < 3 — берём что есть, клонируем последнюю.
    grp_size = 3
    template_group = ph_trs[:grp_size]
    while len(template_group) < grp_size:
        template_group.append(copy.deepcopy(template_group[-1]))

    # Удаляем все placeholder-строки — будем вставлять заново
    for tr in ph_trs:
        tbl_xml.remove(tr)

    # ── Генерируем и вставляем строки по компетенциям ───────────────────────
    for idx, (code, desc) in enumerate(competencies):
        _desc_clean = re.sub(r"^Способен\s+", "", desc, flags=re.IGNORECASE).strip()
        # [Фикс №3] Обрезка по границе слова
        _cut = _desc_clean[:100]
        _cut = _cut[:_cut.rfind(' ')] if len(_desc_clean) > 100 else _cut

        # [Фикс №7] Все 3 подиндикатора в одну ячейку col1 через \n
        indicator_block = (
            f"{code}.1 Знает {_cut}\n"
            f"{code}.2 Умеет применять {_cut}\n"
            f"{code}.3 Владеет навыками {_cut}"
        )

        for r_idx, (otype, items, qualifiers) in enumerate([
            ...
            result_text = f"{prefix} {item}" + (f" {qualifier}" if len(items) == 1 else "")

            # Клонируем нужную строку из template_group (0=restart, 1/2=cont)
            new_tr = copy.deepcopy(template_group[r_idx])

            # Собираем уникальные ячейки строки
            cells: list = []
            seen_tc: set = set()
            for tc in new_tr.findall(f".//{W('tc')}"):
                if id(tc) not in seen_tc:
                    seen_tc.add(id(tc))
                    cells.append(tc)
            ...
            if r_idx == 0:
                # Строка restart: col0=код компетенции, col1=блок индикаторов
                _set_cell_xml(cells[0], code)
                _set_cell_xml(cells[1], indicator_block)
            else:
                # Строки cont: col0/col1 оставляем пустыми (vMerge отображает restart)
                _set_cell_xml(cells[0], "")
                _set_cell_xml(cells[1], "")

            _set_cell_xml(cells[2], result_code)
            _set_cell_xml(cells[3], result_text)
...
def fill_lab_table(doc: Document, lab_works: list, topics: list, hours_lab: int = 18):
    ...
    for i, work in enumerate(lab_works, 1):
        ...
        if isinstance(work, dict):
            ...
            if sec_num and isinstance(sec_num, int) and 1 <= sec_num <= len(sections):
                section = sections[sec_num - 1]
            else:
...
def fill_t6_workload(doc: Document, lec: int, pz: int, lr: int, sro: int,
    ...
    SKIP_PATTERNS = ("on-line", "онлайн", "в т.ч.", "иная", "проектная",
                     ...
                     "контролируем",
                     # [FIX-T6-SRO] Строки СРО, не совпадающие с kw_map,
                     # оставляли «[]» вместо «0» в ячейке «Всего»
                     "изучение учебного", "иные виды")
...
def main(config_path: Optional[str] = None, clear_cache: bool = False):
    ...
    try:
        ...
        # Не прерываем — /models может быть закрыт, сами вызовы могут работать
    ...
    base_vars = {
        ...
        "sections_list":             "",  # заполняется после генерации topics (Фикс №5+6)
    }
    ...
    # [FIX-T11] Добавлена проверка domain drift: если разделы содержат ключевые
    # слова ОНИР/НИРС (наиболее частый источник contamination через retrieval),
    # также применяем fallback независимо от числа разделов.
    _ONIR_KW = {
    ...
    # [Фикс №9] Валидация: каждый раздел должен иметь хотя бы 1 тему.
    # Если LLM вернул только разделы без тем — дополняем базовыми.
    ...
    _sections_list_str = "\n".join(
        f"{i+1}. {re.sub(r'^Раздел\\s*\\d+[.\\s]+', '', s).strip()}"
