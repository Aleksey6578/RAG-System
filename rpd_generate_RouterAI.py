...
# [FIX-З7]
_AI_SOFTWARE_DEFAULTS = [
    "Python 3.x (свободная лицензия)",
    "Jupyter Notebook / JupyterLab (свободная лицензия, BSD)",
    "scikit-learn — библиотека МО (свободная лицензия, BSD)",
    "MATLAB (лицензия УГНТУ)",
    "Jason — среда MAS (GPL)",
]

# [FIX-З12]
_AI_DATABASE_DEFAULTS = [
    "arXiv.org (cs.AI, cs.LG, cs.NE) — открытый доступ",
    "Scopus — реферативная база (лицензия УГНТУ)",
    "IEEE Xplore Digital Library — открытый доступ + лицензия УГНТУ",
    "eLIBRARY.RU — российская научная электронная библиотека",
    "Kaggle Datasets — открытый доступ",
]
    ...
    for attempt in range(3):
        try:
            ...
            if vec:
                ...
                time.sleep(0.2)   # throttle между embed-запросами генератора
                return vec
        except Exception as e:
            if attempt == 2:
                return []
    ...
    try:
        ...
        if not good_hits:
            print(f"    ⚠️  RAG [{section}]: нет чанков выше {GENERATION['min_score']}, "
                  f"пробую без доменного фильтра (section_type сохранён)...")
            # [FIX-OI04] Fallback снимает direction/level, но сохраняет section_type-условия
            # (элементы с "key"="section_type" или структурой "should").
            # Ранее передавался None — retrieval возвращал шапку/метаданные РПД
            # вместо контентных чанков нужного раздела.
            _stf_conds = [c for c in must_conditions
                          if c.get("key") == "section_type" or "should" in c]
            _stf_filter = {"must": _stf_conds} if _stf_conds else None
            vec = get_embedding(queries[0])
            if vec:
                hits = _search_qdrant(vec, _stf_filter, RERANK_TOP_K)  # [FIX-OI04]
...
def fill_t11_sro(doc: Document, topics: list, sro: int, cfg: dict = None):
    ...
    if not (_custom_sro and isinstance(_custom_sro, list) and len(_custom_sro) >= 1):
        # [FIX-06]
        _disc_name = (_cfg.get("discipline", "") or "").lower()  # [FIX-DISCNAME] было "discipline_name" — ключ отсутствует в config.json
...
def _cleanup_artifacts(doc: Document) -> None:
    """

    • Одиночное число (1–3 цифры) в теле документа — артефакт нумерации
      строк/страниц шаблона. Применяется ТОЛЬКО к doc.paragraphs (не к ячейкам
      таблиц), т.к. числа в ячейках (трудоёмкость, номера строк) — валидный контент.
    • Строка ровно «..» — незаполненное поле ФИО (после «Заведующий кафедрой»
      и т.п.). Ищется везде: в теле и в ячейках таблиц.
    """
    _LONE_NUM_RE = re.compile(r"^\d{1,3}$")

    # Одиночные числа — только в теле
    for para in doc.paragraphs:
        if _LONE_NUM_RE.match(para.text.strip()):
            for run in para.runs:
                run.text = ""

    # ".." — тело + ячейки таблиц
    def _clear_dotdot(para):
        if para.text.strip() == "..":
            for run in para.runs:
                run.text = ""
...
def fill_databases_section(doc: Document, cfg: dict) -> None:
    """
    в разделе 6.2 на профессиональные ресурсы домена дисциплины.

    Приоритет источника:
      1. cfg["database_resources"] — явный список строк в config.json
      2. Эвристика по cfg["discipline_focus"] → _AI_DATABASE_DEFAULTS
    """
    resources = cfg.get("database_resources")
    if not resources:
        focus = cfg.get("discipline_focus", "").lower()
        _ai_kw = ("нейрон", "машинн", "интеллект", "обучени", "агент", "нечётк", "алгоритм")
        resources = _AI_DATABASE_DEFAULTS if any(kw in focus for kw in _ai_kw) else None
    if not resources:
        print("  ⚠️  [З-12] database_resources не определён — раздел 6.2 не изменён")
        return

    _BAD_KW = ("microsoft office", "microsoft windows")
    replaced = 0
    for para in doc.paragraphs:
        if replaced >= len(resources):
            break
        if any(kw in para.text.lower() for kw in _BAD_KW):
            if para.runs:
                para.runs[0].text = resources[replaced]
                for run in para.runs[1:]:
                    run.text = ""
            replaced += 1

    if replaced:
        print(f"  ✅ [З-12] Заменено {replaced} записей в разделе 6.2 на AI-ресурсы")
    else:
        print("  ⚠️  [З-12] Microsoft Office/Windows в разделе 6.2 не найдены — проверьте шаблон")


def fill_software_section(doc: Document, cfg: dict) -> None:
    """
    на инструменты, соответствующие домену дисциплины.

    Приоритет источника инструментов:
      1. cfg["software_tools"] — явный список строк в config.json
      2. Эвристика по cfg["discipline_focus"] → _AI_SOFTWARE_DEFAULTS
      3. Если не определён домен — выход без изменений.

    Стратегия замены: paragraph scan по известным bad-строкам (ANSYS/Microsoft),
    чтобы не зависеть от индекса таблицы в шаблоне.
    """
    tools = cfg.get("software_tools")
    if not tools:
        focus = cfg.get("discipline_focus", "").lower()
        _ai_kw = ("нейрон", "машинн", "интеллект", "обучени", "агент", "нечётк", "алгоритм")
        tools = _AI_SOFTWARE_DEFAULTS if any(kw in focus for kw in _ai_kw) else None
    if not tools:
        print("  ⚠️  [З-7] software_tools не определён и домен не распознан — ПО не заменено")
        return

    _BAD_KW = ("ansys", "microsoft office", "microsoft windows")
    replaced = 0
    for para in doc.paragraphs:
        ...
        if any(kw in para.text.lower() for kw in _BAD_KW):
            if para.runs:
                para.runs[0].text = tools[replaced]
                for run in para.runs[1:]:
                    run.text = ""
            replaced += 1

    if replaced:
        print(f"  ✅ [З-7] Заменено {replaced} строк ПО на domain-релевантные инструменты")
    else:
        print("  ⚠️  [З-7] Строки ANSYS/MS Office не найдены в параграфах — проверьте шаблон")
...
def main(config_path: Optional[str] = None, clear_cache: bool = False):
    ...
    # [FIX-З12]
    try:
        fill_databases_section(doc, cfg)
    except Exception as e:
        print(f"  ⚠️  Раздел 6.2 БД: {e}")

    # [FIX-З7]
    try:
        fill_software_section(doc, cfg)
    except Exception as e:
        print(f"  ⚠️  Раздел 7.2 ПО: {e}")

    # [FIX-З6]
    _cleanup_artifacts(doc)
    print("  ✅ Очистка артефактов шаблона")
