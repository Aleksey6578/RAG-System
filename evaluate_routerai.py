"""
evaluate_routerai.py — оценка качества сгенерированного РПД (BLEU + ROUGE).
...
Алгоритм:
  ...
  3. Ищет ближайший эталон в корпусе по cosine similarity
     эмбеддинга названия дисциплины (RouterAI / qwen3-embedding-4b).
  4. Считает BLEU-4 + ROUGE-1/2/L на уровне документа и по секциям.
  5. Сохраняет eval_report.json + печатает таблицу.

Запуск:
  python evaluate_routerai.py                              # defaults
  python evaluate_routerai.py output_rpd.docx --config config.json
  python evaluate_routerai.py --reference rpd_5.json       # принудительный эталон
  python evaluate_routerai.py --ref-docx-dir rpd_docx/     # сравнивать docx-с-docx
"""
...
# ---------------------------------------------------------------------------
# RouterAI — тот же стек что в rpd_generate_RouterAI.py
# ---------------------------------------------------------------------------
ROUTERAI = {
...
# ---------------------------------------------------------------------------
# Embedding + cosine similarity (RouterAI / qwen3-embedding-4b)
# ---------------------------------------------------------------------------
def embed(text: str) -> list[float]:
    """Получает эмбеддинг через RouterAI API с локальным кэшем. [FIX-#9]"""
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                raise RuntimeError(f"embed failed for text[:50]={text[:50]!r}") from e
...
def _normalize_title(title: str) -> str:
    ...
    Это приводит к ненадёжному title-поиску (score 0.478) и необходимости fallback.

    Алгоритм:
    1. Если title содержит ' | ' — разбиваем по разделителю, берём самый длинный
       текстовый (не числовой) фрагмент.
    2. Удаляем оставшиеся одиночные числа и лишние пробелы.
    """
    if not title:
        return title

    # Если содержит табличные разделители
    if " | " in title:
        parts = [p.strip() for p in title.split("|")]
        # Фильтруем: оставляем только нечисловые фрагменты длиной > 3
        text_parts = [p for p in parts if p and not re.match(r"^\d+$", p) and len(p) > 3]
        if text_parts:
            # Берём самый длинный нечисловой фрагмент
            title = max(text_parts, key=len)

    # Удаляем одиночные числа в начале/конце
