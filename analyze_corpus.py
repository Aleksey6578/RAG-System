...
DOMAINS: dict[str, list[str]] = {
    ...
    # [FIX-#22] ИИ/МО перемещён В НАЧАЛО — чтобы «мультиагентн», «экспертн»,
    # «интеллект», «агентн» не перехватывались «Системным ПО» и «Управлением».
    # ==========================================================================
...
def main() -> None:
    ...
    # [FIX-§9] --json: вывод рекомендаций в компактный JSON на stdout.
    # Используется для автоматизации создания config.json-шаблонов:
    #   python analyze_corpus.py --json > gaps.json
    parser.add_argument(
    ...
    # [FIX-§9] --json: компактный JSON с пробелами на stdout для автоматизации
    if args.json:
